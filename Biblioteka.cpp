#include <iostream>
#include <string>					//biblioteka do laczenia string-ow
...
{	cout<<"__________________________WYSZUKIWANIE KSIAZKI_________________________\n";	}

void naglowek_wyszukiwanie_uzytkownika()
{	cout<<"________________________WYSZUKIWANIE UZYTKOWNIKA_______________________\n";  }

void naglowek_spis_ksiazek()
{	cout<<"______________________________SPIS KSIAZEK______________________________\n"; }
...
{
	if(error==false)							//Warunek chroniacy przed przypadkiem krancowym(wyporzyczenie jeżeli poprzednie kroki posiadaly blad)
	{
	char a;
	cout<<user[u].index<<user[u].imie<<user[u].nazwisko<<"\n";
	cout<<"\n1.WYSWIETL INFORMACJE O WYPOZYCZENIACH\n";
			...
			case '1':	
			{
				 if(user[u].wypozyczenia=="; ")
					...
					{		
						...
						cout<<"; WYPOZYCZENIE  DATA ZWROTU: \n"<<user[u].wypozyczenia<<"\n\n";	
						cout<<"KARA W PRZYPADKU ZWROCENIA PO CZASIE: "<<user[u].kara<<"zl\n\n\n";
						system("pause"); system("cls");
						break;
					}
			}
	...
	else;										// W przypadku wczesniejszej deklaracji error=true, program przeleci przez funkcję bez dzialan
}
    ...
    if(dni_miesiaca[miesiac_1]%15==1 && dzien_1<=17)
    ...
    else if(dni_miesiaca[miesiac_1]==28 && dzien_1<=14)
    	{
        	dzien_1+=14;
																...
																//Warunki które pomagaja tworzyc daty zwrotu ksiazki jednakowej dlugosci
		...
		if(pozycja!=string::npos) 		//jesli znaleziono fraze ISBN[n] w user[u].wypozyczenia
			{
				...
				cout << "\n\nNie udalo sie zwrocic ksiazki. Sprawdz, czy uzytkownik na pewno ja wypozyczyl. \n\n";		system("pause"); system("cls");
			}
	}
	else;								// W przypadku wczesniejszej deklaracji error=true, program przeleci przez funkcję bez dzialan
}

void menu_wyp_odd_stan()       	     	//funckja która przy spelnionych warunkach prowadzi do wypozyczenia lub oddania ksiazki
{
...
{	
	...
	cout<<"ISBN: "<<
	spis[n].ISBN<<"\nNazwa: "<<spis[n].nazwa<<"\nAutor: "<<spis[n].autor<<"\nDostepne egzemplarze: "<<spis[n].liczba;
	
	}
	else								// Warunek który komunikuje zle wpisany index
		{
			error=true;					// Zmiana bool error na true (kolejne kroki zostana wylaczone, program powroci do menu bez wprowadzania zmian
			cout<<"Podano nieprawidlowy ISBN\n";
			system("pause"); system("cls");
		}
}
...
void menu()
{
	...
	cout<<"2.WYSZUKAJ UZYTKOWNIKA\n";
	cout<<"3.WYSWIETL SPIS KSIAZEK\n";
	cout<<"4.WYJSCIE\n";
...
void ilosc_fizyczna_ksiazek()					//funkcja wczytujaca ilosc magazynowa ksiazek
{
	...
	if(plik_fizyczna.good()==false)				//sprawdzenie poprawnosci pliku
		{
			cout<<"PROGRAM NAPOTKAL PROBLEM Z PLIKIEM ILOSC_FIZYCZNA_KSIAZEK.TXT, SPRAWDZ PLIK I URUCHOM BIBLIOTEKE PONOWNIE";
			exit(0);
		}
...
void wczytanie_uzytkownikow()				//Funkcja sterujaca wczytaniem informacji o poszczegolnych uzytkownikach	
{
	...
	if(plik_user.good()==false)				//sprawdzenie poprawnosci pliku
		{
			cout<<"PROGRAM NAPOTKAL PROBLEM Z PLIKIEM INDEX.TXT, SPRAWDZ PLIK I URUCHOM BIBLIOTEKE PONOWNIE";
			exit(0);
		}
...
void wczytanie_ksiazki()						//Funkcja sterujaca wczytaniem informacji o poszczegolnych ksiazkach
{
	...
	if(plik_ksiazki.good()==false)				//sprawdzenie poprawnosci pliku
		{
			cout<<"PROGRAM NAPOTKAL PROBLEM Z PLIKIEM KSIAZKI.TXT, SPRAWDZ PLIK I URUCHOM BIBLIOTEKE PONOWNIE";
			exit(0);
		}
